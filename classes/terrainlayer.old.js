...
export class TerrainLayer extends CanvasLayer{
	constructor(){
        ...
        this.terrainhud = new TerrainHUD();
    }

    get hud() {
        return this.terrainhud;
    }
    ...
    */
    ...
    highlightGridPosition(layer , {gridX, gridY, multiple=2,type='ground'}={}) {
        ...
        let bit = (s / 16) * (Math.clamped(multiple, 2, 4) - 1);
        let mid = (s / 2);
    ...
    _onDragLeftMove(e){
        if (game.activeTool == "select" ) return this._onDragSelect(e);
    }
    ...
    _onDragLeftDrop(e) {
        ...
        const isSelect = ["select"].includes(tool);
        if (isSelect) {
...
class TerrainHUD extends BasePlaceableHUD {
